...
void SpiralField::ActivateMedium(const double& timeOfNow, const double& frameTime)
{
	...
	for (int i = 0;i < NumberOfRay;++i)
	{
		...
		{
			//设定时间与角度有关，形成螺旋效果
			MediumLayer[MediumPixelIndex].GetActivated(timeOfNow+2*(double)i/NumberOfRay/this->getSourceSpeed(), this->getSourceAmplitude(), this->getSourceFrequency(), this->getSourceInitialPhase());
